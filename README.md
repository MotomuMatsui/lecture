...
### `1_penguins.ipynb` — Palmer Penguins で機械学習一巡り
...
- **次元圧縮による可視化**
  - PCA / MDS(PCoA)/ t-SNE / UMAP の4手法を並べて、線形手法と非線形手法を比較
...
### `3_pLM.ipynb` — タンパク質言語モデル（ProtBERT）入門
...
**学習のねらい**: ProtBERTは「事前学習済み」モデルであり、その埋め込みベクトルさえ手に入れば**シンプルな線形分類器でも高精度に分類できる**という事実を体感します。

### `4_gpcr.ipynb` — GPCR-リガンド親和性予測（創薬への入口）

**G蛋白質連関受容体（GPCR）とペプチドリガンドの相互作用予測** にProtBERT埋め込みを応用するノートブックです。GPCRは全医薬品の約35%が標的とする最重要受容体ファミリーで、本ノートブックで、**ペアの配列から結合/非結合を予測する**という、創薬の中核タスクへ向けた第一歩を踏み出します。
...
**学習のねらい**: `3_pLM.ipynb` で学んだ「単一タンパク質の埋め込み」を「**タンパク質ペアの埋め込み**」へと拡張し、相互作用予測（PPI予測）の枠組みを体感します。
...
**学習のねらい**: 「遺伝的距離」「近隣結合法vs.最尤法」「ブートストラップによる信頼度評価」という系統解析の三本柱を、鳥類進化に思いを馳せながら身につけます。
